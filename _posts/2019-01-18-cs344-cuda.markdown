...
Ideas to try:  
- Load the image patch and filter weights into the shared memory. 
...
- Try using shared memory for calculating a histogram for a thread block then copy the result to the global memory. 
