...
It's interesting that (X * Y) + Z calculated on CPU doesn't equal (X * Y) + Z calculated on GPU, because GPU uses Fused Multiply-Add operation (FMA). So CPU version looks like `Round(Round(X * Y) + Z)` and GPU with FMA `Round(X * Y + Z)`. This moment may result in small difference between CPU and GPU output. In the kernel above I use explicit rounding to match the reference (CPU) result.  
